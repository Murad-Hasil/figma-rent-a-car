...
function PickDropForm({ className = '' }) {
  return (
    <div className={`flex flex-col md:flex-row justify-center items-center w-full max-w-[1200px] mx-auto mt-8 gap-6 ${className}`}>
    ...
    <div className='h-fit pt-32 flex flex-col md:flex-row'>
      ...
      <div className='w-full md:w-[80%] flex flex-col items-center'>
        ...
        <div className='w-full max-w-[1200px] grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 p-4'>
          {cars.map((car, index) => (
            <div key={index} className='relative bg-white p-4 rounded-[10px] shadow-lg w-full h-auto'>
              ...
              <Image src={car.image} alt={car.name} width={1000} height={1000} className='w-full h-auto object-cover rounded-md mb-4' />
              <div className='flex flex-wrap items-center space-x-2 mb-4'>
                <Image src='/gas-station.png' alt='Fuel Icon' width={20} height={20} />
                <span className='text-gray-500 text-sm'>{car.fuelCapacity}</span>
                ...
                <span className='text-gray-500 text-sm'>{car.seatingCapacity}</span>
              </div>
              <div className='flex justify-between items-center'>
                <div className='text-black text-sm font-bold mb-4'>
                  ${car.price.toFixed(2)}/ <span className='text-gray-500'>day</span>
                  {car.oldPrice && <><br /><del className='text-gray-400'>${car.oldPrice.toFixed(2)}</del></>}
                </div>
